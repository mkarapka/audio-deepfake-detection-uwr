...
class LogisticRegressionObjective(Objective):
    ...
    def __call__(self, *, trial: Trial, epochs: int, use_pos_weight: bool, logging_percent_threshold: float = 0.1):
        classifier = LogisticRegressionClassifier(in_features=self.in_features)

        lr = trial.suggest_float("lr", 1e-5, 1e-2, log=True)
        weight_decay = trial.suggest_float("weight_decay", 1e-5, 1e-2, log=True)
        if use_pos_weight:
            pw = trial.suggest_float("pos_weight", 5.0, 40.0)
            pos_weight = torch.tensor([pw], device=classifier.device)
        else:
            pos_weight = None
...
class MlpObjective(Objective):
    ...
    def __call__(self, *, trial: Trial, epochs: int, use_pos_weight: bool, logging_percent_threshold: float = 0.1):
        ...
        hidden_sizes = self._get_suggested_hidden_sizes(trial=trial)
        classifier = MlpClassifier(input_size=self.in_features, hidden_sizes=hidden_sizes, dropout_rate=dropout_rate)
