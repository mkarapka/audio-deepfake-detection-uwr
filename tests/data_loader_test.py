...
class DataLoaderTest:
    ...
    def test_load_all_splits(self, MockFeatureLoader):
        ...
        for split in splits_to_test:
            dataset = DataLoader(split=split, feat_suffix="dummy", device="cpu")

            assert len(dataset) == 4, f"Expected length 4, got {len(dataset)}"

            feat_0, label_0 = dataset[0]
            assert torch.allclose(feat_0, torch.tensor([0.1, 0.2], dtype=torch.float32)), "Features of first element don't match"
            assert label_0.item() == 1.0, f"Expected label_0 to be 1 (bonafide), got {label_0.item()}"

            feat_1, label_1 = dataset[1]
            assert torch.allclose(feat_1, torch.tensor([0.3, 0.4], dtype=torch.float32)), "Features of second element don't match"
